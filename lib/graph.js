export class Graph {
  ...
  getDistanceBetween(vertexKey1, vertexKey2) {
    ...
    return vertex1 && vertex2 ? this.calculateDistance(vertex1.x, vertex1.y, vertex1.z, vertex2.x, vertex2.y, vertex2.z) : Infinity;
  }
  ...
  getEdges() {
    ...
    this.vertices.forEach((vertex, key) => {
      vertex.getNeighbors().forEach(neighbor => {
        edges.push({ from: key, to: neighbor.node.createCompositeKey(), distance: neighbor.dist });
      });
    });
    return edges;
  }
...
export class Node {
  ...
  printNeighbors() {
    this.neighbors.forEach((vertex) => {
      console.log(
        `Neighbor: ${vertex.node.createCompositeKey()} =-= Distance: ${
          vertex.dist
        }`
      );
    });
