...
export class SceneObject {
  constructor(defaultColor, defaultLineWidth, boundingBox) {
    ...
    this.mapWidth = Math.abs(this.eastBound - this.westBound) * 1600;
    this.mapHeight = Math.abs(this.northBound - this.southBound) * 1600;
  }

  // Add a node formatted with mercator project points
  addNode(node) {
    this.nodes.set(node.id, {
      projected: convertToXY(node.lat, node.lon, this.mapWidth, this.mapHeight),
